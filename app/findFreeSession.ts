...
export async function findFreeSession(data, conflict): Promise<freeSession> {
  ...
  let endTimeEpoch =
    Date.parse(
      data.events[conflictIndex1].date +
        "T" +
        data.events[conflictIndex1].hour +
        ":" +
        data.events[conflictIndex1].minute +
        ":00"
    ) +
    Number(data.events[conflictIndex1].duration) * 60000;
  ...
  while (foundSession == false) {
    ...
    if (result.totalConflicts == 0) {
      ...
      foundHour =
        newDate.getUTCHours() < 10
          ? "0" + newDate.getUTCHours().toString()
          : newDate.getUTCHours().toString();
      foundMinute =
        newDate.getUTCMinutes() < 10
          ? "0" + newDate.getUTCMinutes().toString()
          : newDate.getUTCMinutes().toString();
